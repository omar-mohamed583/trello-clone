...
export function deleteSection(boardId, sectionId) {
  ...
  boards.nodesCount -= section.nodes.length;
  board.content.splice(board.content.indexOf(section), 1);
...
export function addBoard(userId, title) {
  ...
  saveBoard();
  const todoId = addSection(boardId, 'Todo');
  const inProgId = addSection(boardId, 'In Progress');
  const doneId = addSection(boardId, 'Done');

  // Mark default sections as pinned
  [todoId, inProgId, doneId].forEach(id => {
    const sect = boards.boardsData[getBoardIndex(boardId)]?.content.find(s => s.id === id);
    if (sect) sect.pinned = true;
  });
  saveBoard();
  return boardId;
}
