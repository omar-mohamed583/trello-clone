import { changeNodePosition, changeSectionsOrder, getBoard, getNode, getSection, swapNodesBetweenSections, swapNodesInSameSection, users } from "../Data/boards.js";
...
const updateClonedSectPos = throttle(coords => {
  ...
  clonedSect.style.top = coords.clientY - offset.y + 'px';
  clonedSect.style.left = coords.clientX - offset.x + 'px';
});

const updateClonedNodePos = throttle(coords => {
  if (!clonedNode) return;
  clonedNode.style.top = coords.clientY - offset.y + 'px';
  clonedNode.style.left = coords.clientX - offset.x + 'px';
});
...
document.body.addEventListener('pointermove', e => {
  if (clonedSect || clonedNode) {
    ...
    if ((clientX <= mainContainerRect.right) &&
      ...
      (clientY <= mainContainerRect.bottom) &&
      (clientY >= mainContainerRect.top)
    ) {
      if (clonedNode || clonedSect) {
        const coords = { clientX, clientY };
        if (clonedSect) {
          updateClonedSectPos(coords);
        } else if (clonedNode) {
          updateClonedNodePos(coords);
        }
      }
    } else {
      clonedSect?.remove();
...
function createClonedEle(ele, e) {
  ...
  clonedEle.style.position = 'fixed';
